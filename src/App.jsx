...
export default function App() {
  ...
  const addToCart = useCallback((p) => {
    setCart(prev => { const next = [...prev, p]; saveLocal("cart", next); return next; });
    setToast(`Added "${p.title}"`);
    setTimeout(() => setToast(null), 2000);
  }, []);
  const removeFromCart = useCallback((i) => {
    setCart(prev => { const next = prev.filter((_, idx) => idx !== i); saveLocal("cart", next); return next; });
  }, []);
    ...
    <div style={{ minHeight: "100vh", background: P.abyss, color: P.ghost, position: "relative" }}>
      ...
      {isMobile && !isHome && (
        <nav style={{
          ...
          }}>{location.pathname.replace("/", "").replace(/-/g, " ") || "Home"}</div>
          <a href="/cart" style={{
      ...
      <div style={{ position: "relative", zIndex: 2, animation: isMobile ? "none" : "morphBreath 1.5s ease-in-out infinite", willChange: isMobile ? "auto" : "opacity" }} data-protected>
        <Routes>
          <Route path="/" element={isMobile ? <MobileHub cartCount={cart.length} /> : <Hero />} />
          <Route path="/portfolio" element={<Portfolio addToCart={addToCart} portfolioTab={portfolioTab} setPortfolioTab={setPortfolioTab} />} />
          <Route path="/portfolio/:id" element={<ShowcaseDetail addToCart={addToCart} />} />
          <Route path="/portfolio/design/:id" element={<CaseStudyDetail />} />
